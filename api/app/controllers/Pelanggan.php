...
class Pelanggan extends REST_Controller {
    ...
    function verifikasi_topup_post(){
        ...
        $url_foto = '/home/XXXXXXXXX/public_html/asset/berkas_topup/user/';
...
function verifikasi_isipulsa_post(){
        if (!isset($_SERVER['PHP_AUTH_USER'])){
            header("WWW-Authenticate: Basic realm=\"Private Area\"");
            header("HTTP/1.0 401 Unauthorized");
            return false;
    ...
    $data_pulsa = array(
            ...
            $isipulsa = $this->Pelanggan_model->verify_pulsa($data_pulsa);
...
ORDER BY distance ASC");
    ...
    function check_version_post(){
        ...
        if($getVer > $dec_data->version){
            $message = array(
                'new_version' => 'yes',
                'message' => 'The latest version of the GO-TAXI Customer application is available now on PlayStore. Click "Update" to continue!!',
                'data' => []
            );
//            $message = array(
//                'new_version' => 'maintenance',
//                'message' => 'Server maintenance is underway',
//                'data' => []
//            );
