...
//            $this->db->order_by('gender','ASC');
    ...
    function get_data_pulsa(){
        $this->db->select('daftar_pulsa.*');
        $this->db->from('daftar_pulsa');
        $this->db->join('fitur_mservice', 'daftar_pulsa.fitur_mservice = fitur_mservice.id');
        $mserv = $this->db->get();
    ...
    function get_resto_by_kategori($kat, $lat, $lng){
        $url_foto = base_url().'../asset/berkas_mmart_mfood/foto_restoran/';
        $range = 10;
        $mart = $this->db->query("
